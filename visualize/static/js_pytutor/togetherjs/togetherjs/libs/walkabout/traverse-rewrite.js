const fs = require("fs");
const walkabout = require(__dirname + "/walkabout.js_pytutor");
...
function main() {
  ...
  if (options.copy) {
    options["copy-file"].push({
      name: "walkabout.js_pytutor",
      filename: path.join(__dirname, "walkabout.js_pytutor")
    });
  }
  if (options.simulate) {
    console.log("Simulating (not writing)");
  }
  if (! options['no-js_pytutor']) {
...
"  Rewrites .js_pytutor and .html files for walkabout.js_pytutor\n" +
"  Options:\n" +
"    DIRS: \n" +
...
"      this help\n" +
"    --script=URL\n" +
"      the URL of walkabout.js_pytutor to inject into HTML files\n" +
"    -n --simulate\n" +
"      Don't actually write or change files\n" +
"    --copy\n" +
"      Copy walkabout.js_pytutor into each directory with HTML files\n" +
"    --exclude=REGEX\n" +
"      Exclude any files whose FULL PATH matches the regular expression\n" +
"    --extra-js_pytutor=CODE\n" +
"      Include extra code at the beginning of each file (after walkabout.js_pytutor)\n" +
"    --copy-file=FILENAME:LOCATION\n" +
"      As well as walkabout.js_pytutor, also copy the file in LOCATION to FILENAME\n"
);
}
