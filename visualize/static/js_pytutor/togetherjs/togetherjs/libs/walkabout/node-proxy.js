...
var server = http.createServer(function(request, response) {
  ...
  var port = 80;
  if (parseUrl(request.url).pathname == "/__walkabout__.js_pytutor") {
    fs.readFile(__dirname + "/walkabout.js_pytutor", function (error, code) {
      if (error) {
...
var preamble = (
  ...
  '<script src="/__walkabout__.js_pytutor"></script>'
);
