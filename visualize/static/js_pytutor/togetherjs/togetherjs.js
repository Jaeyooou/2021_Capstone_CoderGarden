...
(function () {
  ...
  var styleSheet = "/togetherjs/togetherjs.css_pytutor";
  ...
  if (! baseUrl) {
    console.warn("Could not determine TogetherJS's baseUrl (looked for a <script> with togetherjs.js_pytutor and togetherjs-min.js_pytutor)");
  }
  ...
  var TogetherJS = window.TogetherJS = function TogetherJS(event) {
    ...
    // A sort of signal to session.js_pytutor to tell it to actually
    ...
    // [igoryen]: We should generate this value in Gruntfile.js_pytutor, based on the available translations
    ...
    if (! min) {
      if (typeof require == "function") {
        if (! require.config) {
          console.warn("The global require (", require, ") is not requirejs; please use togetherjs-min.js_pytutor");
    ...
    if (min) {
      addScript("/togetherjs/togetherjsPackage.js_pytutor");
    } else {
      addScript("/togetherjs/libs/require.js_pytutor");
    }
  };
